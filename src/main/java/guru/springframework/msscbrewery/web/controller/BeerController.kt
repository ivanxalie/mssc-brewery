...
class BeerController(private val beerService: BeerService) {
    ...
    @PostMapping
    fun addBeer(@RequestBody beerDto: BeerDto): ResponseEntity<Any> {
    ...
    @PutMapping("/{beerId}")
    fun updateBeer(@RequestBody beerDto: BeerDto, @PathVariable("beerId") beerId: UUID): ResponseEntity<Any> {
